...
impl OptRandStateMachine {
    ...
    pub(crate) fn on_new_timeout_event(&mut self, 
        ev: TimeOutEvent, 
        ev_queue: &mut EventQueue, 
    ) -> Result<()> {
        match ev {
            ...
            TimeOutEvent::StopAcceptingAck(e) => {
                log::info!("Time to stop accepting acks in {}", e);
                self.stop_accepting_acks(e)
            }
