...
use crate::{EventQueue, MsgBuf, NewMessage, OptRandStateMachine as OSM, ev_queue, events::{Event, TimeOutEvent}};
...
async fn test_proposal() -> Result<()> {
    ...
    let mut leader_osm = osms.pop_front()
        .ok_or("Unable to pop an OSM".to_string())?;
    let mut ev_queue: Vec<_> = (0..N).map(|i| {
        EventQueue::with_capacity(100_000)
    }).collect(); 
    let mut msg_buf:Vec<_> = (0..N).map(|i| {
        MsgBuf::new()
    }).collect();
    log::info!("Starting the test");
    ...
    assert_eq!(new_leader_osm.is_leader(), true, "The first node should be the leader in epoch 1");
    ...
    if let ProtocolMsg::Propose(x, y) = &msg {
        ...
        let ev = ev_queue[0].events_queue().pop_back().ok_or("Non leader node did not add the proposal to the event queue".to_string())?;
