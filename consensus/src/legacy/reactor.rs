...
) 
{
    ...
    let mut cx = Context::new(config, net_send, sh_out, verify_in);
    let delta = cx.delta();
    ...
    loop {
        tokio::select! {
            pmsg_opt = net_recv.recv() => {
                ...
                cx.ev_queue.push_back(Event::NewMsgIn(sender, msg));
            }
            phase = delay_queue.next() => {
                let phase = phase.unwrap().unwrap().into_inner();
                cx.ev_queue.push_back(phase);
            }
