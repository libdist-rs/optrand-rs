...
use crate::{Certificate, Codeword, DbsContext, DirectProposal, Epoch, MTAccumulator, MTAccumulatorBuilder, Replica, RespCertData, RespCertProposal, Storage, SyncCertData, SyncCertProposal, Vote, Witness, error::Error};
use crypto::{DSSPublicKey, hash::{Hash, ser_and_hash, EMPTY_HASH}};
use fnv::FnvHashMap;
...
impl<T> std::default::Default for Proposal<T> 
where T: Default,
{
    fn default() -> Self {
        Self {
...
impl WireReady for Proposal<RespCertData> {
    fn init(self) -> Self {
        self
    }
...
impl DirectProposal {
    ...
    pub fn is_valid(&self, 
        ...
        proof.sign.buffered_is_valid(&(e,proof.acc.clone()), pk_map, storage)
    }
}
...
impl RespCertProposal {
    ...
    pub fn is_valid(&self, 
        ...
        proof: &Proof<Self>,
        storage: &mut Storage,
        resp_cert_acc_builder: &MTAccumulatorBuilder<Self>,
        pk_map: &FnvHashMap<Replica, DSSPublicKey>,
    ) -> Result<(), Error> {
        // Are all the signatures valid
        self.data.cert.buffered_is_valid(&self.data.vote, pk_map, storage)?;
        ...
        // Is the accumulator valid?
        resp_cert_acc_builder.check(self, &proof.acc)?;
