...
cargo build --package=node-optrand-opt --release
echo "Starting protocol nodes"

DELTA=${DELTA:-"50"}

sudo perf record ./target/$TYPE/node-optrand-opt -c $TESTDIR/nodes-0.dat -d ${DELTA} -i ./scripts/ip_file $1 &> 0.log&
...
# perf ./target/$TYPE/node-optrand-opt "-c $TESTDIR/nodes-0.dat -d ${DELTA} -i ./scripts/ip_file $1" &> 0.log
