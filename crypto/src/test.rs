...
#[cfg(test)]
mod ctx_tests {
    use crate::{DbsContext, Keypair, PublicKey, Scalar, std_rng};
    use ark_bls12_381::Bls12_381;
    ...
    fn test_gen() {
        ...
        let _ = DbsContext::<E>::new(&mut rng, h2, h1, n, t, 0, public_keys, keypairs[0].0);
    }
    ...
    fn test_generation() {
        ...
        let dbs_ctx = DbsContext::<E>::new(&mut rng, h2,h1, n, t, 0, public_keys, keypairs[0].0);
        ...
        let a1: PublicKey<E> = dbs_ctx.public_keys[0].mul(w.into_repr());
...
mod dbs_tests {
    ...
    fn agg_test() {
        ...
        let dbs_ctx:Vec<_> = (0..n).map(|i| {
            DbsContext::<E>::new(&mut rng, h2,h1, n as usize, t, i, public_keys.clone(), secret_keys[i as usize].clone())
        }).collect();
    ...
    fn decryption_test() {
        ...
        let dbs_ctx:Vec<_> = (0..n).map(|i| {
            DbsContext::<E>::new(&mut rng, h2, h1, n, t, i, public_keys.clone(), secret_keys[i])
        }).collect();
        let pvec = 
    ...
    fn test_reconstruction() {
        ...
        let gs_orig = dbs_ctx[0].optimizations.g2p.mul(s.into_repr());
        assert_eq!(beacon.value, gs_orig);
        assert_eq!(beacon.beacon, E::pairing(dbs_ctx[0].h1p, beacon.value));
    }
}
