...
macro_rules! bench_curve {
    ($pairing_curve:ty, $pairing_name: literal) => {
        ...
        pub fn pvss_generation(c: &mut Criterion) {
            ...
            for &n in &TEST_POINTS {
                ...
                let h1 = G1::prime_subgroup_generator().mul(Scalar::<E>::rand(&mut rng));

                let dbs_ctx = DbsContext::<E>::new(&mut rng, h2, h1, n, t, 0, public_keys, secret_keys[0]);
        ...
        pub fn pvss_verification(c: &mut Criterion) {
            let mut group = c.benchmark_group(
                format!("{}_pvss_verification", $pairing_name)
            );
            BenchmarkGroup::sampling_mode(&mut group, criterion::SamplingMode::Flat);
            for &n in &TEST_POINTS {
        ...
        pub fn pvss_aggregation(c: &mut Criterion) {
            let mut group = c.benchmark_group(format!("{}_pvss_aggregation", $pairing_name));
            BenchmarkGroup::sampling_mode(&mut group, criterion::SamplingMode::Flat);
            for &n in &TEST_POINTS {
                ...
                let dbs_ctx:Vec<_> = (0..t+1).map(|i| {
                    DbsContext::<E>::new(&mut rng, h2, h1, n, t, 0, public_keys.clone(), secret_keys[i].clone())
                }).collect();
        ...
        pub fn pvss_pverify(c: &mut Criterion) {
            let mut group = c.benchmark_group(format!("{}_pvss_pverify", $pairing_name));
            BenchmarkGroup::sampling_mode(&mut group, criterion::SamplingMode::Flat);
            for &n in &TEST_POINTS {
        ...
        pub fn pvss_decomposition_verify(c: &mut Criterion) {
            let mut group = c.benchmark_group(format!("{}_pvss_decomposition_verify", $pairing_name));
            BenchmarkGroup::sampling_mode(&mut group, criterion::SamplingMode::Flat);
            for &n in &TEST_POINTS {
        ...
        pub fn pvss_decryption(c: &mut Criterion) {
            let mut group = c.benchmark_group(format!("{}_pvss_decryption", $pairing_name));
            BenchmarkGroup::sampling_mode(&mut group, criterion::SamplingMode::Flat);
            for &n in &TEST_POINTS {
        ...
        pub fn pvss_verify_decryption(c: &mut Criterion) {
            let mut group = c.benchmark_group(format!("{}_pvss_decryption_verify", $pairing_name));
            BenchmarkGroup::sampling_mode(&mut group, criterion::SamplingMode::Flat);
            for &n in &TEST_POINTS {
                ...
                let dbs_ctx:Vec<_> = (0..n).map(|i| {
                    DbsContext::<E>::new(&mut rng, h2, h1, n, t, i, public_keys.clone(), secret_keys[i].clone())
                }).collect();
                let mut pvecs = Vec::with_capacity(t+1); 
                for i in 0..t+1 {
                    let pvec = 
        ...
        pub fn pvss_reconstruction(c: &mut Criterion) {
            let mut group = c.benchmark_group(format!("{}_pvss_reconstruction", $pairing_name));
            BenchmarkGroup::sampling_mode(&mut group, criterion::SamplingMode::Flat);
            for &n in &TEST_POINTS {
